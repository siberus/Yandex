...
auto Solve2x2SystemOfLinearEquations(double a, double b, double c, double d, double e, double f, const double& EPS = 1e-5) {
    vector<double> solution;
    double determinant = a * d - b * c;
    if (abs(determinant) < EPS) {
        if (abs(a) < EPS && abs(c) < EPS) {
            if (abs(b) < EPS && abs(d) < EPS) {
                if (abs(e) < EPS && abs(f) < EPS) return make_pair(InfinitelyManySolutions_AnyXYPair, solution);
                else return make_pair(NoSolution, solution);
            } else if (e * d == f * b) {
        ...
        double x, y;
        if (abs(d) < EPS) x = (f - d * e / b) / (c - d * a / b), y = (e - a * x) / b;
        else x = (e - b * f / d) / (a - b * c / d), y = (f - c * x) / d;
